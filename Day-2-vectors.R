...
# Let's say we want a function to return the index of
# the first element with a '3' value of any vector:
...
third <- function(x){
  ...
  cat("Index of first '3' value is: ")
  return(i)
}

# If there is no '3' element value, it returns the index
# of the last element in the vector, regardless of value.
...
# vector (1,0,0,1,1,1,0,1,1) has run of length 3 starting at
# position 4, and runs of length 2 beginning at indices 4, 5 and 8.
...
#### Unfortunately no data with this example so we go quickly
...
# Get error rate for k = 3. Then do k = 1, k = 2, etc up to some
# maximum value of k. Then use one value of k that works best
# in future.
...
preda <- function(x,k) {
  ...
  for (i in 1:(n-k)) {
    # heart of program: are predicting day i+k from k days previous
    # and storing result in pred[i] so we count 1's in those days
    # which is sum(x[i:(i+(k-1))])
    if (sum(x[i:(i+(k-1))]) >= k2) pred[i] <- 1 else pred[i] <- 0 
  }
  # pred has predicted values; x[(k+1):n] has actual values for
  # days in question; subtracting gives us -1,0, or 1. Only 0
  # is a good prediction; abs() gets rid of -1's, still 0 is good one.
  return(mean(abs(pred-x[(k+1):n])))
  # mean() gives us the proportion
}

# preda.r does a lot of duplicate computation. For successive values
# of i in the loop, sum() is being called on vectors that differ
# by only two elements. Except when k is very small, this slows
# things down a lot.

# So we re-write it so that at each iteration of the loop
# we update the previous sum, instead of computing a new sum
# "from scratch"
## Program predb.r
predb <- function(x,k) {
  n <- length(x)
  ...
  if (n-k >= 2) {
    for (i in 2:(n-k)) {
      ...
      # oldest element making up the sum (x[i-1]) and adding the
      # new one (x[i+k-1])
...
# What about vectorized functions that appear to use 'scalars':
f <- function(x,c) return((x+c)^2)
f(1:3,0)
f(1:3,1)

# we intended for c to be scalar but this works too
f(1:3,1:3) # ends up adding the elements of the two vectors first
...
# form is ifelse(b,u,v) where b Boolean vector, u and v are vectors
...
findud <- function(v) {
  ...
  vud <- v[-1] - v[-length(v)]  # subtracts each element from previous
  # into 1's and -1's
...
udcorr <- function(x,y) {
  ...
  # which are treated as 1 and 0 values by mean and gives us our fraction:
...
# return(mean(ud[[1]] == ud[[2]])) above returns fractions of matches
...
### AN EXTENDED EXAMPLE: RECODING AN ABALONE DATA SET
